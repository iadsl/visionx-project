...
class SE(nn.Module):
    def __init__(self, in_channels, channels, se_ratio=12):
        ...
        reduced_channels = max(48, channels // se_ratio)  # Ensure minimum channels for SE block
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Conv2d(in_channels, reduced_channels, kernel_size=1, padding=0),
...
class LinearBottleneck(nn.Module):
    def __init__(self, in_channels, channels, t=6, stride=1, use_se=True, se_ratio=12, dropout=0.0):
        ...
        ConvBNSiLU(out, in_channels, dw_channels, kernel=1)

        out.append(nn.Conv2d(dw_channels, dw_channels, kernel_size=3, stride=stride, padding=1, groups=dw_channels, bias=False))
        out.append(nn.BatchNorm2d(dw_channels))
        out.append(SiLU(inplace=True))

        if use_se:
            out.append(SE(dw_channels, dw_channels, se_ratio))

        ConvBNSiLU(out, dw_channels, channels, kernel=1)
...
class ReXNetV1(nn.Module):
    def __init__(self, input_ch=3, final_ch=180, width_mult=1.0, depth_mult=1.0, classes=2,
        ...
        stem_channel = 96 if width_mult >= 1.0 else int(96 * width_mult)

        layers = [1, 2, 2, 2]  
        strides = [1, 2, 2, 2]
        use_ses = [False, True, True, True]  
        ts = [1] + [6] * (sum(layers) - 1)  
        ...
        inplanes = stem_channel  
        features = []
        ConvBNSiLU(features, 3, stem_channel, kernel=3, stride=2, pad=1)

        for block_idx in range(len(strides)):  
            out_channels = int((inplanes + final_ch / (self.depth // 3)) * width_mult)  
        ...
        pen_channels = max(640, int(1280 * width_mult))  
