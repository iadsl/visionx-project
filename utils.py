...
def init_distributed_mode(args):
    ...
    if "MASTER_ADDR" not in os.environ:
        os.environ["MASTER_ADDR"] = "127.0.0.1"  # Default to localhost
    if "MASTER_PORT" not in os.environ:
        os.environ["MASTER_PORT"] = "29500"  # Default PyTorch port

    print(f' Distributed Init | Rank: {args.rank} | GPU: {args.gpu} | URL: {args.dist_url}')
...
class NativeScalerWithGradNormCount:
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        ...
        if update_grad:
            if clip_grad is not None:
                ...
                self._scaler.unscale_(optimizer)
                norm = self._get_grad_norm_(parameters)
    ...
    @staticmethod
    def _get_grad_norm_(parameters, norm_type: float = 2.0) -> torch.Tensor:
